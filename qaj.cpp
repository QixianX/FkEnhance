...
//Mat CalculateEntropyFromImage(Mat img,Mat result, string name)
...
int main()
{
	string path = "G:/cell/hxdtest/1_8_-/*.jpg"; 
	vector<String> imageFiles;
	glob(path, imageFiles);
	cout << imageFiles.size() << endl;
    //string path = "G:/cell/hxdtest/20210413172143.jpg";
	//clock_t end1 = clock();
	//cout << "dutu:" << end1 - start << "ms\n";
	for (unsigned int pic = 0; pic < imageFiles.size(); ++pic)
	{
		cout << imageFiles[pic] << endl;
		clock_t start = clock();
		Mat img = imread(imageFiles[pic]);
		...
		//Mat v = channels.at(2);
		//clock_t end2 = clock();
		//cout << "fenli:" << end2 - end1 << "ms\n";

		//FileStorage fs("G:/cell/hxdtest/build/1.5.xml", FileStorage::READ);
		//Mat mat_vocabulary;
		//fs["1.5"] >> mat_vocabulary;
		//clock_t el = clock();
		//cout << "lxml:" << el - end2 << "ms\n";
		string bl, gr, re;
		...
		//cvtColor(dstImage, dstImage, CV_HSV2BGR);
		//clock_t end5 = clock();
		//cout << "hebing:" << end5 - end4 << "ms\n";
		clock_t end = clock();
		cout << "亮度调整耗时为:" << end - start << "ms\n";
		vector<string> v;
		SplitString(imageFiles[pic], v, "\\");
		cout << "split done!" <<v.size() << endl;
		cout << "G:/cell/hxdtest/build/result/" + v[1] << endl;

		//imshow("HJ", dstImage);
		imwrite("G:/cell/hxdtest/build/re-r/"+ v[1], dstImage);
		/*imwrite(imageFiles[pic], dstImage);*/
		//waitKey(0);
